...
std::string premissionsToString(bool isDir,std::filesystem::path path)
{
	...
	if ((perm & std::filesystem::perms::others_exec) != std::filesystem::perms::none)
		...
		premissions += "-";
	//-----TODO: FIX LINK COUNT LATER
	premissions += (isDir == 0 ? " 2 " : " 1 "); //if its dir add placeholder 2 else add placeholder 1 
...
{
	char flag;
	if (input.length() > 3)//TODO: LATER CHECKS WILL NOT CHECKY BY LENGTH BUT BY A MAP OF COMMANDS
	{
